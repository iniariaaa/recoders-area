...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------AriaGanteng'+'ARIA--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'AriaApi') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'AriaApi') return res.json(loghandler.invalidKey)
...
router.get('/ppcp', async (req, res, next) => {
        var apikeyInput = req.query.apikey
            
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'AriaApi') return res.json(loghandler.invalidKey)

       fetch(encodeURI(`https://raw.githubusercontent.com/Caliph71/txt/main/couple.json`))
...
router.get('/randomquote', async (req, res, next) => {
	...
	if(apikeyInput != 'AriaApi') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`https://mhankbarbar.herokuapp.com/api/randomquotes`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/randomnekonime', async (req, res, next) => {
        var apikeyInput = req.query.apikey
            
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'AriaApi') return res.json(loghandler.invalidKey)

       fetch(encodeURI(`https://mhankbarbar.herokuapp.com/api/nekonime`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/nulis', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	 ...
	 if(!text) return res.json(loghandler.nottext)
         rest = await fetch(`http://zekais-api.herokuapp.com/bukukanan?text={text}`)
...
router.get('/xnxx', async (req, res, next) => {
    ...
    if (!text) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter text"})
       ...
       fetch(encodeURI(`https://alpin-api-2021.herokuapp.com/api/xnxxVidio?text=${text}&apikey=alpin1`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/call', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'AriaApi') return res.json(loghandler.invalidKey)
    if (!no) return res.json({ status : false, creator : `${creator}`, message : "masukan nomer hp"})
...
router.get('/dewabatch', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'AriaApi') return res.json(loghandler.invalidKey)
        if(!judul) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter q"})

       fetch(encodeURI(`https://mhankbarbar.herokuapp.com/api/dewabatch?q=${judul}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/yutub/search', async (req, res, next) => {
    ...
    if (!video) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter video"})
       ...
       fetch(encodeURI(`https://api.zeks.xyz/api/yts?apikey=AriaGanzTzy&q=${video}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
